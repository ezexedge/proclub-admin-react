...
import {getNotificacionByUsers} from 'src/api/notificaciones'
...
function PageHeader() {
  ...
  const llamadaUsuariosXNotificiacion = async(param) => {
    const result =  await getNotificacionByUsers(param,token)
    setUsuarios(result)

    console.log('todos lo usuarios',result)
  }
...
//getTematicaByNotificacion
  ...
  useEffect(() => {
    ...
    */
      ...
      if(userId){
        llamadaUsuariosXNotificiacion(userId)
      }
...
<Box component={RouterLink} to="/admin/dashboard/notificacion" className={classes.boxBtn}>
    <KeyboardBackspaceIcon className={classes.icon}/>
</Box>
<Typography variant="h1" mt={0.5} ml={3}>{notificacionSingle && transformText(notificacionSingle.titulo)}</Typography>
  </Grid>
...
<Box mt={3}>
<Typography variant="h2" >{notificacionSingle && notificacionSingle.descripcion}</Typography>
</Box>
<Grid xs={8}>
<Typography  className={classes.textSecondary} color="secondary" >
{notificacionSingle &&  parse(notificacionSingle.descripcion_corta)}
</Typography>
</Grid>
...
<Typography color="secondary" >{'Revisá la lista antes de enviar'}</Typography>
    ...
    <Card >
      <Results users={usuarios !== null ? usuarios : []} />
        </Card>
    </Grid>
