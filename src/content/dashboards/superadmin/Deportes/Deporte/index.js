...
import {getDeporteID,getDeportesEstadistica} from 'src/api/deportes'
import {
  Avatar,
  Box,
  LinearProgress,
...
const Deporte = () => {
    ...
    const llamadaId = async () => {
      const result = await getDeporteID(userId,token)
      console.log(result)
      setDeporte(result)
    }

    const llamadaEstadisticas =  async(param) => {
      const result =  await getDeportesEstadistica(param)
    ...
    const imageDeporte = (param) => {
      ...
      return( 
      <img style={{ maxWidth: 50}} src={result.icono} />
      )
     }
...
>
  ...
  {deporte &&  imageDeporte(deporte.icono)}
...
<Box m={1} />
<Typography mb={1} className={classes.cardTitle}>
                  Clubes vinculados
              </Typography>

              <Box m={0.5} />
    <div className={classes.cardSubtitle} >{estadisticas && estadisticas.club}<span className={classes.cardSubtitle2}></span></div>
...
<Typography  mb={1} className={classes.cardTitle}>
              ...
              <Box m={0.5} />
    <div className={classes.cardSubtitle}>{estadisticas && estadisticas.socios}<span className={classes.cardSubtitle2}></span></div>
...
<Typography  mb={1} className={classes.cardTitle}>
              ...
              <Box m={0.5} />
    <div className={classes.cardSubtitle}>{estadisticas && estadisticas.manager}<span className={classes.cardSubtitle2}></span></div>
