...
import { deleteClub } from 'src/api/club'
...
const applyFilters = (users, query, filters) => {
  return users.filter((user) => {
    ...
    if (query) {
      const properties = ['nombre','email','telefono'];
...
const Results = ({ users,actualizarClub, ...rest }) => {
  ...
  const handleDeleteCompleted = async() => {
    setOpenConfirmDelete(false);
      ...
      const result = await deleteClub(eliminarId,token)
          ...
          {paginatedUsers.length === 0 ? (
            <>
              <Typography
                ...
                align="center"
              >
                {t("No pudimos encontrar ningún club que coincida con sus criterios de búsqueda")}
              </Typography>
            </>
          ) : (
            <>
                ...
                <Table>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell align="center">{t('Telfono')}</TableCell>
                    ...
                    {paginatedUsers.map((user) => {
                      ...
                      return (
                        <TableRow hover key={user.id} selected={isUserSelected}>
                          ...
                          <TableCell>
                            <Typography>{user && user.direccion.calle} {user && user.direccion.numero}</Typography>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <Typography>{ user && user.persona !== null ?  user.persona.nombre : 'no hay datos'} {user && user.persona !== null ?  user.persona.apellido : ''}</Typography>
                          </TableCell>
                          <TableCell>
...
<Label color="success" >Activo</Label>
                            ...
                            <Typography noWrap>
                              <Tooltip title={t('Ver')} arrow>
                                <IconButton
                                  ...
                                  to={'/superadmin/dashboard/club/' + user.id}
                                  color="primary"
      ...
      {!toggleView && (
        <Card sx={{ textAlign: 'center', p: 3 }}>
          ...
          >
            {t(
              'Elija entre vistas de tabla o cuadrícula para mostrar la lista de clubs.'
            )}
          </Typography>
        </Card>
      )}
