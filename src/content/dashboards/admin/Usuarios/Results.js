...
const applyFilters = (users, query, filters) => {
  return users.filter((user) => {
    ...
    if (query) {
      const properties = ['nombre','apellido', 'telefono','documento'];
      let containsQuery = false;

      properties.forEach((property) => {

        if(user.usuario.persona[property] !== null){
        if (user.usuario.persona[property].toLowerCase().includes(query.toLowerCase())) {
          containsQuery = true;
        }
      }

      if(user.rol[property] !== null){
        if (user.rol['nombre'].toLowerCase().includes(query.toLowerCase())) {
          containsQuery = true;
        }
      }
      });
...
const Results = ({ users, ...rest }) => {
  ...
  useEffect(()=>{
    ...
    const llamadaDeportes = async () => {
      let  result = await getAllDeportesXUsuario()
    //  console.log(result)
      setDeportes(result)
    }
  ...
  const handleDeleteCompleted = async() => {
    ...
    await deleteUsuario(135,eliminarId)
                          ...
                          <TableCell>
                            <Box display="flex" alignItems="center">
                              ...
                              <Box  className={classes.deporte}>
                                  {user && user.usuario.persona !== null ? user.usuario.persona.nombre : ''}  {user && user.usuario.persona !== null ?  user.usuario.persona.apellido : 'no hay datos'} 
                                
                              </Box>
                            </Box>
                          </TableCell>
                          <TableCell>
                            <Typography>{user && user.rol.nombre}</Typography>
                          </TableCell>
                          <TableCell>
                            <Typography>{user && user.usuario.persona !== null ? user.usuario.persona.documento : 'no hay datos'}</Typography>
                          </TableCell>
                          <TableCell align="center">
                            <Typography >
                          {user && user.usuario.persona !== null ? user.usuario.persona.telefono : 'no hay telefono' }
                            </Typography>
                          </TableCell>
                          <TableCell nowrap>
                              <Typography  className="deportesUsuarios">futbol tenis</Typography>
                          </TableCell>
                          <TableCell>
                         
<Label color={user && estadoColor(user.estado.nombre)} >

{user && user.estado.nombre}
